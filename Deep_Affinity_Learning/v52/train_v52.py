...
import math
import myModel_v52
import myMultimodalDataset_v52
...
if args.milestone:
	...
	model = myModel_v52.MultimodalNetwork()
	starting_epoch = 0

model = model.cuda()
criterion = myModel_v52.AffinityLoss().cuda()
# print(model)


train_dataset = myMultimodalDataset_v52.myMutimodalDataset(gndPKLPath=dataset_dir + 'train_fold'+str(fold)+'.pkl', phase="train")
train_dataloader = DataLoader(train_dataset, shuffle=True, num_workers=0, batch_size=train_batch_size)

# train_dataset = myMultimodalDataset_v52.myMutimodalDataset(gndPKLPath=dataset_dir + 'test_fold'+str(fold)+'.pkl', phase="train")
# train_dataloader = DataLoader(train_dataset, shuffle=True, num_workers=0, batch_size=train_batch_size)

val_dataset = myMultimodalDataset_v52.myMutimodalDataset(gndPKLPath=dataset_dir + 'test_fold'+str(fold)+'.pkl', phase="test")
