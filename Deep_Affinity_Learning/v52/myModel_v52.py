...
class AffinityLoss_test(nn.Module):
    ...
    def forward(self, input, target, mask0, mask1):
        ...
        # input_pre = input_next = (input_pre + input_next) / 2
        average = (input_pre[:, :, :self.Nm_phone, :self.Nm_camera] + input_next[:, :, :self.Nm_phone, :self.Nm_camera]) / 2
        input_pre[:, :, :self.Nm_phone, :self.Nm_camera] = average
        input_next[:, :, :self.Nm_phone, :self.Nm_camera] = average
