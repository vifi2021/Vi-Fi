...
# sequence_names = sorted([d for d in os.listdir('/media/hans/WINLAB-MOBILE-GROUP/RAN_Dec_data/') if '2020122' in d and os.path.isdir(os.path.join('/media/hans/WINLAB-MOBILE-GROUP/RAN_Dec_data/',d))])
...
for seq_id, sequence_name in enumerate(sequence_names):
	...
	# if sequence_name != '20201229_162235': continue
	# if seq_id != 14: continue
	sequence_dir = os.path.join('/media/hans/WINLAB-MOBILE-GROUP/RAN_Dec_data/', sequence_name)
	zedbox_output_dir = sequence_dir + '/zedBox_3D_' + sequence_name + '.txt'
	...
	output_file_name = 'zedBox_3d_gnd_match.txt'
	...
	for t in range(len(labeled_frames)):
		...
		for i in range(len(labeled_frames[t]['regions'])):
			...
			img = cv2.rectangle(img, (int(bbox_x), int(bbox_y)), (int(bbox_x)+int(bbox_w)-2, int(bbox_y)+int(bbox_h)-2), color=(0, 255, 0)) 
